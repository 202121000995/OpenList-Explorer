...
- 内置 OpenList：首次启动可选择使用随软件释放的 OpenList，默认监听 `127.0.0.1:15244`，并自动启动、连接和读取管理信息；管理端会用系统浏览器打开，显示的是网页登录密码，不是 API Token。
...
- 云端下载：支持调用 OpenList 离线下载任务，并同步任务状态；安装包随带 Aria2，可一键连接本机 RPC，再到 OpenList 管理端启用 Aria2 工具。
...
一键连接 Aria2 会启动本机 RPC，并直接使用软件“下载目录”设置。要让云端下载任务真正使用 Aria2，还需要在 OpenList 管理端的离线下载工具设置里启用 Aria2，并把 Aria2 工具指向这个 RPC 地址。软件会在云下载弹窗和设置页提供复制配置与打开管理端入口。

桌面端启动内置组件时会释放为专属进程名：

- `openlist_explorer_builtin.exe`
- `aria2_explorer_builtin.exe`

这样软件关闭或下次启动时只认领自己的内置组件，不会误关用户单独安装的 OpenList 或 Aria2。
