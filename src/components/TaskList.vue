  ...
  },
  {
    ...
    render(row) {
      return h(NSpace, { justify: 'end', size: 6 }, () => [
        ...
        h(
          NButton,
          { circle: true, size: 'small', secondary: true, onClick: () => emit('pause', row.id) },
          { icon: () => h(Pause, { size: 15 }) }
        ),
        h(
          NButton,
          { circle: true, size: 'small', secondary: true, onClick: () => emit('resume', row.id) },
          { icon: () => h(Play, { size: 15 }) }
        ),
        h(
          NButton,
          { circle: true, size: 'small', secondary: true, type: 'error', onClick: () => emit('cancel', row.id) },
          { icon: () => h(X, { size: 15 }) }
        ),
        h(
          NButton,
